...
def assemble_signature(password: bytes, salt: bytes):
    ...
    # print(f'unencoded password and salt {salted}')
    # print(f'str.encode password and salt {str.encode(salted)}')
    signature = hashlib.sha256()
    # signature.update(str.encode(salted))
    signature.update(salted)
    # print(f'Return from assemble_signature {signature.digest()}')
...
class RequestHandler(socketserver.StreamRequestHandler):
    ...
    def _register(self, ip:str, port: int, signature: bytes):
        ...
        try:
            ...
            print(f'saveable_sig {saveable_sig}')
                ...
                if peer not in [my_address, new_address]:
                    ...
                    server.send_to_server(
                        assemble_message(
                            ...
                            msg), 
                        expect_reply=False)
                    # print(f'Informing {server.port} about {port} with salt {salt}')
                    # print(f'Sending {saveable_sig.hex()}')
    ...
    def _handle_request(self, ip:str, port: int, signature: bytes, 
        ...
        for network_address, network_signature, network_salt , _ in network:
            if network_address == address:
                ...
                # print(f'Peer validation')
                # print(f'Testable signature {testable_signature.hex()}')
                # print(f'Network signature {network_signature.hex()}')
                # print(f'Signature (recieved?) {signature.hex()}')
                # print(f'Network salt {network_salt}')
...
if __name__ == "__main__":
    ...
    signature = assemble_signature(str.encode(password), str.encode(salt))
    print(f'Startup: {signature} salt {salt}')
